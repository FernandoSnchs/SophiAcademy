...
$users_ler = fopen($users, 'r'); // Lê o conteúdo do arquivo

while (($line = fgets($users_ler)) !== false) {
    $line = trim($line); // Remove espaços em branco no início e no fim da linha
    if (!empty($line)) { // Verifica se a linha não está vazia
        $proc = "Email: '{$email}'";
...
<body>
    ...
    <div class="content">
        ...
        <div class="erro">
            ...
            <div class="txt2">
                <p>Verificar Email e senha</p>
            </div>
            <div class="txt3">
                <p>se não tem uma contra ainda <a href="./cadastro.html">clique aqui!</a></p>
            </div>
            <a href="./login/login.html" class="button"><button class="button">voltar</button></a>
        </div>
